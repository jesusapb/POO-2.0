...
public class CtrlPerfil implements ActionListener {
                        ...
                        if (mat.equals(var.getMatricula()) && pass.equals(var.getContraseña())) {
                            if (vp.ed_nombre.isSelected() == true) {
                                try {
                                    ...
                                    ps = con.prepareStatement("UPDATE usuarios SET nombre = ? WHERE matricula = '" + mat + "'");
                            ...
                            } else if (vp.ed_ap_pat.isSelected() == true) {
                                try {
                                    PreparedStatement ps = null;

                                    ModConexion objCon = new ModConexion();
                                    Connection con = objCon.getConexion();
                                    ps = con.prepareStatement("UPDATE usuarios SET ap_pat = ? WHERE matricula = '" + mat + "'");

                                    ps.setString(1, appart);
                                    ps.execute();

                                    var.setAp_pat(appart);
                                    var.setNombre_completo(npart + " " + appart + " " + ampart);
                                    va.txtNombre.setText(var.getNombre_completo());
                            ...
                            } else if (vp.ed_ap_mat.isSelected() == true) {
                                try {
                                    PreparedStatement ps = null;

                                    ModConexion objCon = new ModConexion();
                                    Connection con = objCon.getConexion();
                                    ps = con.prepareStatement("UPDATE usuarios SET ap_mat = ? WHERE matricula = '" + mat + "'");

                                    ps.setString(1, ampart);
                                    ps.execute();

                                    var.setAp_mat(ampart);
                                    var.setNombre_completo(npart + " " + appart + " " + ampart);
                                    va.txtNombre.setText(var.getNombre_completo());
                                ...
                                vp.ed_ap_pat.setEnabled(false);
                                vp.ed_ap_mat.setEnabled(false);
    ...
    public void limpiar() {
        ...
        vp.ed_ap_pat.setSelected(false);
        vp.ed_ap_mat.setSelected(false);
        ...
        vp.ed_ap_pat.setEnabled(true);
        vp.ed_ap_mat.setEnabled(true);
